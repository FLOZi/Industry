...
Worker.SetIdle = function(self)
	--Spring.Echo("Worker", self.name, "is now idle")
	local building = GetObjectFromID(self.workplace)
	table.insert(building.idleWorkers, self.id)
	self.idle = true
end

Worker.TaskAllocated = function(self, task)
	table.copy(task, self.currTask) -- TODO: is this funciton even needed?
end
...
Task.Create = function (self, cmdQ, limit, name)
	local new = Task:New()
	new.cmdQ = cmdQ -- Does this need to be a table.copy()?
	new.limit = limit
	new.name = name
	return new
end
...
Building.AssignWorker = function (self, worker)
	if self.workerCount < self.maxWorkers then
		...
		table.insert(self.idleWorkers, worker.id)
...
Building.OrderWorker = function (self, cmdID, cmdParams, cmdOpts, limit)
	if self.workerCount > 0 then
		--local availableWorker = self.idleWorkers[1]
		...
		if availableWorkers > 0 then
			--Spring.GiveOrderToUnit(availableWorker, cmdID, cmdParams, cmdOpts or table.empty)
			Spring.GiveOrderToUnitArray({unpack(self.idleWorkers, 1, limit)}, cmdID, cmdParams, cmdOpts or table.empty)
			--table.remove(self.idleWorkers, 1) -- TODO: implement some smarter stack instead?
...
Yard.AddSite = function(self, siteID)
	...
	local costsTask = Task:Create(costsCmdQ, site.maxBuilders, "costs")
	--site.costsTask.building = self
	--site.costsTask = costsTask
	costsTask.building = self
	if self.idleWorkers[1] then
		for i = 1, math.min(site.maxBuilders, #self.idleWorkers) do
			...
			costsTask:AllocateWorker(self.idleWorkers[i])
		end
	end
	site.costsTask = costsTask
	--site.buildTask = Task:Create(buildCmdQ, site.maxBuilders, "build")
	local buildTask = Task:Create(buildCmdQ, site.maxBuilders, "build")
	--site.buildTask.building = self
	buildTask.building = self
	site.buildTask = buildTask
end

Yard.TaskFinished = function(self, task)
	if task.name == "costs" then
		if self.idleWorkers[1] then
			local currSite = self:CurrentSite()
			local buildTask = currSite.buildTask
			for i = 1, math.min(task.limit, #self.idleWorkers) do
				buildTask:AllocateWorker(self.idleWorkers[i])
			end
		end
